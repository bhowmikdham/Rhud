...
describe('documentToRawPoints — wide multi-application questionnaire', () => {
  ...
  it('treats an asset INVENTORY (count | count | type columns) as single-column, not multi-app (Link-18)', () => {
    ...
    // read as an appId and "Application/Operations Server | 23" was split from
    // its descriptor, so the 23 servers were never priced.
    const doc = blankDoc();
    doc.sheets = [
